...
print(t() + "[info] Running directory scan to the target server.")
...
for i in range(maxlen):
    ...
    if rcode < 400:
        ...
        num = num + 1
        reply = str(input(bcolors.BOLD + ' Do you want to continue scan for more possible results? (y/n): ')).lower().strip()
        print(bcolors.ENDC)
        if reply == 'n':
            final_result()
