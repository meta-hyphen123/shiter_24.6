...
def detect_suspicious_activity():
    while True:
        # 在这里放置检测可疑活动的代码逻辑
        # 如果检测到可疑活动，触发断网操作
        time.sleep(5)  # 5秒检测一次，可以根据需要调整间隔
...
detect_thread.daemon = True  # 设置为守护线程，主程序结束时自动结束
detect_thread.start()
...
def install_tool(tool_name, tools):
    ...
    if tool_name in tools:
        ...
        # Use the `git clone` command to download the tool into the `user` directory
        subprocess.run(f"git clone {url} {git_folder} --force", shell=True)
def process(user_input):
    ...
    if '1' == '1':
            ...
            if user_input == "dir":
                ...
                elif user_input.startswith("shiter install "):
